        ...
        if current_row == new_row || current_col == new_col
            ...
            if new_row.between?(0, 7) && new_col.between?(0, 7)
                ...
                if current_row == new_row
                    ...
                    range = current_col < new_col ? (current_col+1..new_col) : (new_col..current_col-1)
                    range.each do |col|
                ...
                if current_col == new_col
                    range = current_row < new_row ? (current_row+1..new_row) : (new_row..current_row-1)
                    range.each do |row|
                        return false unless board[row][current_col] == "."
                    end
                end

                # If no obstacles were found, the move is valid
                return true
            end
        end
